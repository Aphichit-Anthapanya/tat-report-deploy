...
import { fetchMasterDataSection1Service, setSection1ByName } from '@/redux/OperationFollow/Section1/service';
...
export default function Page() {
    ...
    useEffect(() => {
        ...
        if(mode == 1){
            setSection1ByName('organizationManageMent','แผนสนับสนุนดำเนินงานด้านการตลาด',dispatch);
        }else if(mode == 2){
            setSection1ByName('organizationManageMent','แผนสนับสนุนในประเทศ',dispatch);
        }else if(mode == 3){
            setSection1ByName('organizationManageMent','แผนสนับสนุนต่างประเทศ',dispatch);
        }        
    ...
    return (
        ...
        <div className="breadcrumb-zone">นำเข้าข้อมูล {'>'} <span className="bread-crumb-page-name">ข้อมูลโครงการด้านบริหารจัดการองค์กร </span> {'>'} <span className="bread-crumb-page-name">งานระบบสารสนเทศแบบองค์กร</span></div>
            ...
            <div className="flow-indicator d-flex justify-content-center">
                ...
                <div onClick={() => handleSection(1, true)} className={`circle active`}>1</div>
                <div className="section-zone-text">ข้อมูลโครงการงานระบบสารสนเทศองค์กร</div>
                </div>
                <div className="flow-line-zone"><hr /></div>
                <div className="flow-section-zone">
                <div onClick={() => handleSection(2,true)}  className={`circle ${
                    sectionNumber != 1 
                 ? 'active' : ''}`}>2</div>
                ...
                <div className="flow-line-zone"><hr /></div>
                <div className="flow-section-zone">
                <div onClick={() => handleSection(3,true)}  className={`circle ${
                    sectionNumber != 2 &&
                    sectionNumber != 1  
                ? 'active' : ''}`}>3</div>
                ...
                <div className="flow-line-zone"><hr /></div>
                <div className="flow-section-zone">
                <div onClick={() => handleSection(4,true)}  className={`circle ${
                    sectionNumber != 3 &&
                    sectionNumber != 2 &&
                    sectionNumber != 1 
                ? 'active' : ''}`}>4</div>
                ...
                <div className="flow-line-zone"><hr /></div>
                <div className="flow-section-zone">
                <div onClick={() => handleSection(5,true)} className={`circle ${
                    sectionNumber != 4 &&
                    sectionNumber != 3 &&
                    sectionNumber != 2 &&
                    sectionNumber != 1 
