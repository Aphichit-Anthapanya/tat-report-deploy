...
export default function Section2(props: Section2Props){
    ...
    const handleQuarterSelect = (value:number) => {setQuarterSelect(value)}
    ...
    const formatCurrency = (name:string) => {
        let formattedValue = ''
        if(name == 'budget'){
            formattedValue = Number(formState['section2']['budget']).toLocaleString('en-US', {
    ...
    const fetchMask = () => {
        setBudgetTemp(Number(formState.section2.budget).toLocaleString('en-US', {
            minimumFractionDigits: 2,
            maximumFractionDigits: 2,
        }))
    }
    ...
    return (
        ...
        <div className="d-flex form-group">
                ...
                <label>งบประมาณโครงการ<span style={{color:'red'}}> *</span></label>
                </div>
                <div className="d-flex flex-column form-field-fieldzone">
                    <div className="d-flex">
                        ...
                        <input onBlur={(e) => formatCurrency('budget')} onChange={(e) => handleChangeField(e)} name="budget" value={formState.section2.budget} type="text" className="form-control hide" id="exampleFormControlInput1" placeholder=""/>
                        <input onBlur={(e) => onBlurTempMask(e)} onChange={(e) => onChangeTempMask(e)} name="budget" value={budgetTemp} type="text" className="form-control"/>
                        </div>
            ...
            <div className="d-flex form-group">
                <div className="form-field-label">
                <label>งบประมาณแยกตามรายเดือน<span style={{color:'red'}}> *</span></label>
                </div>
                <div className="d-flex flex-column form-field-fieldzone">
                    <div>
                        { validationState[0].isValid == false && 
                        <div className="form-field-fieldzone">
                            <span style={{color:'red'}}>โปรดกรอกข้อมูล{validationState[0].alias} ให้ครบ</span>
                        </div>
                        }
                    </div>
                    <div className="">
                    <TableQuarterSummary onChangeQuarter={handleChangeQuarter} quarterSelect={quarterSelect} setQuarterSelect={handleQuarterSelect} tempMask={tempMask} setTempMask={handleSetTempMask} />
                    </div>
                </div>
            </div>
            <div className="d-flex form-group">
                <div className="form-field-label">
                <label>ความสอดคล้องกับนโยบาย<span style={{color:'red'}}> *</span> <br /> (เลือกได้มากกว่า 1)</label>
                </div>
                <div className="d-flex flex-column form-field-fieldzone">
                    <div>
                        { validationState[2].isValid == false && 
                        <div className="form-field-fieldzone">
                            <span style={{color:'red'}}>โปรดเลือก{validationState[2].alias} อย่างน้อย 1 ค่า</span>
                        </div>
                        }
                    </div>
                    <div className="">
            ...
            <div className="d-flex form-group">
                <div className="form-field-label">
                <label>กลุ่มผู้มีส่วนได้ส่วนเสีย<span style={{color:'red'}}> *</span> <br /> (เลือกได้มากกว่า 1)</label>
                </div>
                <div className="d-flex flex-column form-field-fieldzone">
                    <div>
                        { validationState[3].isValid == false && 
                        <div className="form-field-fieldzone">
                            <span style={{color:'red'}}>โปรดเลือก{validationState[3].alias} อย่างน้อย 1 ค่า</span>
                        </div>
                        }
                    </div>
                    <div className="">
            ...
            <div className="button-section">
                <button  onClick={() => handleSection(3)} type="button" className="btn btn-primary">ต่อไป <i className="bi bi-chevron-right"></i></button>
                <button type="button" className="btn btn-primary"><i className="bi bi-file-earmark"></i> บันทึกข้อมูลชั่วคราว </button>
                <button onClick={() => handleSection(1)} type="button" className="btn btn-primary"><i className="bi bi-chevron-left"></i> ย้อนกลับ </button>
            </div>
        </>
    )
}